<template>
  <div
    class="p-4 sm:p-6 lg:p-4 bg-gradient-to-b from-slate-50 to-slate-100 min-h-screen font-sans"
  >
    <div class="mx-auto max-w-7xl">
      ...
      <div class="mb-8">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <div
              class="p-3 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg"
            >
              <svg
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 mt-1">
                Estadísticas e Información en Tiempo Real
              </p>
            </div>
          </div>
        </div>
      ...
      <div
        v-if="visitStore.loading || reservationStore.loading"
        class="flex justify-center items-center p-12"
      >
        <div class="text-center">
          <div class="inline-block relative w-20 h-20 mb-4">
            <div
              class="absolute border-4 border-gray-200 rounded-full w-20 h-20"
            ></div>
            <div
              class="absolute border-4 border-blue-500 rounded-full w-20 h-20 border-t-blue-600 animate-spin"
            ></div>
          </div>
      ...
      <div
        v-else-if="visitStore.error || reservationStore.error"
        class="bg-red-50 border-l-4 border-red-500 p-6 rounded-lg"
      >
        <div class="flex items-center">
          ...
          >
            <path
              ...
              stroke-linejoin="round"
              stroke-width="2"
              d="M12 9v2m0 4v2m0 0v-6m0 6H9m3 0h3M9 5h6a2 2 0 012 2v12a2 2 0 01-2 2H9a2 2 0 01-2-2V7a2 2 0 012-2z"
            />
          </svg>
          <div>
            <p class="font-semibold text-red-700">Error al cargar datos</p>
            <p class="text-sm text-red-600">
              {{ visitStore.error || reservationStore.error }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <!-- Total Histórico -->
          <div
            class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-6 border-l-4 border-indigo-500"
          >
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm font-medium">Total Histórico</p>
                <p class="text-3xl font-bold text-gray-900 mt-2">
                  {{ visitStore.dashboardStats.totalVisits ?? 0 }}
                </p>
              </div>
          ...
          <!-- Visitas Activas -->
          <div
            class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-6 border-l-4 border-green-500"
          >
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm font-medium">Visitas Activas</p>
                <p class="text-3xl font-bold text-gray-900 mt-2">
                  {{ visitStore.dashboardStats.activeVisits ?? 0 }}
                </p>
              </div>
          ...
          <!-- Visitas Hoy -->
          <div
            class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-6 border-l-4 border-amber-500"
          >
            <div class="flex items-center justify-between">
              <div>
                <p class="text-gray-600 text-sm font-medium">Visitas de Hoy</p>
                <p class="text-3xl font-bold text-gray-900 mt-2">
                  {{ visitStore.dashboardStats.visitsToday ?? 0 }}
                </p>
              </div>
          ...
          <!-- Reservas Totales -->
          <div
            class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 p-6 border-l-4 border-purple-500"
          >
            <div class="flex items-center justify-between">
              <div>
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="mb-4">
              <h3 class="text-lg font-semibold text-gray-900">
                Reservas: Vigentes vs Expiradas
              </h3>
              <p class="text-sm text-gray-500 mt-1">
                Análisis del estado de reservas pendientes
              </p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="mb-4">
              ...
              <p class="text-sm text-gray-500 mt-1">
                Visitas registradas por hora del día
              </p>
            </div>
            <div class="relative h-72">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="mb-4">
              ...
              <p class="text-sm text-gray-500 mt-1">
                Top 10 de áreas con mayor visitas
              </p>
            </div>
            <div class="relative h-72">
        ...
        <div
          class="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg shadow-sm p-6"
        >
          <div class="flex items-center space-x-4">
            ...
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900 mt-1">
                {{ visitStore.dashboardStats.mostVisitedArea ?? "N/A" }}
              </p>
              <p class="text-sm text-gray-600 mt-1">
                {{
                  visitStore.dashboardStats.mostVisitedAreaCount ?? 0
                }}
                visitas en total
              </p>
            </div>
...
const computeChartsData = () => {
  ...
  const reservations = JSON.parse(
    JSON.stringify(reservationStore.reservations || []),
  );
  ...
  const activas = visits.filter(
    (v) => v.estado === "activa" && !isVisitExpired(v),
  ).length;
  const finalizadas = visits.filter((v) => v.estado === "finalizada").length;
  const vencidas = visits.filter(
    (v) => isVisitExpired(v) && v.estado !== "finalizada",
  ).length;
  ...
  const hourData = hourLabels.map(
    (lbl) => hourCounts[lbl.substring(0, 2)] || 0,
  );
